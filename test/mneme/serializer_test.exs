defmodule Mneme.SerializerTest do
  ...
  describe "patterns" do
    ...
    test "pins and guards" do
      ...
      auto_assert "^my_ref" <- to_pattern_strings(ref, binding: [my_ref: ref])

      self = self()
      auto_assert "pid when is_pid(pid)" <- to_pattern_strings(self)
      auto_assert "^me" <- to_pattern_strings(self, binding: [me: self])

      {:ok, port} = :gen_tcp.listen(0, [])

      try do
        auto_assert "port when is_port(port)" <- to_pattern_strings(port)
        auto_assert "^my_port" <- to_pattern_strings(port, binding: [my_port: port])
      after
        Port.close(port)
      end
    end
    ...
    test "dates and times" do
      ...
      auto_assert "^jan1" <- to_pattern_strings(datetime, binding: [jan1: datetime])
      ...
      auto_assert "^jan1" <- to_pattern_strings(naive_datetime, binding: [jan1: naive_datetime])

      date = Date.from_iso8601!(iso8601_date)
      auto_assert "~D[2023-01-01]" <- to_pattern_strings(date)
      auto_assert "^jan1" <- to_pattern_strings(date, binding: [jan1: date])

      time = Time.from_iso8601!(iso8601_time)
      auto_assert "~T[12:00:00]" <- to_pattern_strings(time)
      auto_assert "^noon" <- to_pattern_strings(time, binding: [noon: time])
    end
  ...
  defp to_pattern_strings(value, context \\ []) do
    value
    |> Serializer.to_patterns(Enum.into(context, %{}))
